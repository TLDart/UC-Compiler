...
ls -a "$3"| grep  "\.c\|.uc" | while read -r line ; do
		...
		then
			...
			gcc codegen.c lex.yy.c y.tab.c free.c functions.c print_lib.c symbol_table.c semantics.c -o uccompiler && 
			...
			result=`./uccompiler < $3/$line > temp.ll && lli temp.ll | diff $3/$processed -`
			rm temp.ll
			if [ "$result" ]; 
			then
				echo "Error found "
